...
import { getCustomerAge } from 'api/reportApi';
import numeral from 'numeral';
import { useEffect, useState } from 'react';

const ReportCustomerAge = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getCustomerAge(searchValue);
        setData(result?.data);
      } catch (error) {
        setError(error);
      } finally {
  ...
  return (
    <div>
      ...
      />
       <h4>Khách hàng có tuổi  {numeral(searchValue).format('0,0')}đ</h4>
      <Table
        rowKey="_id"
        columns={columns}
        dataSource={data}
...
export default ReportCustomerAge;